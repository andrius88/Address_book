...
import lt.vcs.addresbook.storage.indbutils.JdbcAddresEntry;
...
import lt.vcs.addressbook.storage.javasqlconnection.JavaSqlConnAddressBook;

public class ConsoleApplicationJavaSqlConn { 
	...
	public static void main(String[] args) {
		...
		IAddresBookStorage addressbook = new JavaSqlConnAddressBook();
	...
	public static void add(String[] params, IAddresBookStorage addressbook) {
		if (InputDataValidator.isValidParamCount(4, params)) {
			...
			if (!InputDataValidator.isValidName(name)) {
				...
				IAddresEntry entry = new JdbcAddresEntry();
				entry.setName(name);
				entry.setEmail(email);
				entry.setMobilePhone(mobilePhone);
				try {
					addressbook.save(entry);
					System.out.println("A new entry was added to DB sucessfuly");
				} catch (Exception e) {
					e.printStackTrace();
					System.out.println("A new entry was NOT added to DB");
				}
			}
		} else {
	...
	public static void removeById(String[] params, IAddresBookStorage addressbook) {
		...
		if (InputDataValidator.isValidParamCount(2, params)) {
			...
			try {
				...
				list = addressbook.findEntryById(removeId);
				if (list.isEmpty()){
					System.out.println("Entry with ID: " + removeId + " doesn't exist, please try valid ID "
							+ "(Show_all command)");
				} else {
					try {
					...
					if (wasRemoved) {
						System.out.println("Entry with ID:" + removeId + " was removed from DB");
					} else {
	...
	public static void findByMobNumber(String[] params, IAddresBookStorage addressbook) {
		if (InputDataValidator.isValidParamCount(2, params)) {
			...
			if (!InputDataValidator.isValidMobPhone(phoneNoIn)) {
				...
				if (list.isEmpty()){
					System.out.println("The Entry having phone no.: " + phoneNoIn + " was not found");
				} else {
					System.out.println("Lits by name was recieved and will be printed");
					pintList(list);
				}
	...
	public static void findById(String[] params, IAddresBookStorage addressbook) {
		if (InputDataValidator.isValidParamCount(2, params)) {
			...
			if (!InputDataValidator.isValidId(idInStr)) { // checks is ID is a
				...
				System.out.println("Id has to be some integer number existing in the list (Show_all command)");
			} else {
				int idInInt = Integer.parseInt(idInStr);
				List<IAddresEntry> list = new ArrayList<IAddresEntry>();
				list = addressbook.findEntryById(idInInt);
				if (list.isEmpty()){
					System.out.println("The entry with Id: " + idInInt + 
							" was not found, please enter valid Id (Show_all command)");
				} else {
					pintList(list);
				}
			}
	...
	public static void editById(String[] params, IAddresBookStorage addressbook) {
		if (InputDataValidator.isValidParamCount(5, params)) {
			...
			if (!InputDataValidator.isValidId(idInStr)) { // checks if ID is a number
				...
				int idInInt = Integer.parseInt(idInStr);  // current ID as integer
				IAddresEntry entry = new JdbcAddresEntry();
				entry.setId(idInInt);
				entry.setName(name);
				entry.setEmail(email);
				...
				try {
					addressbook.updateById(entry);
					System.out.println("The entry having ID: " + idInInt + " was updated in DB sucessfuly");
				} catch (Exception e) {
					e.printStackTrace();
					System.out.println("The entry having ID: " + idInInt + " was not updated in DB");
				}
			}	
		} else {
